...
class ViewRequests(ViewBase):
    def __init__(self, main_window, ep_window, requests, ep_model):
        super().__init__(self)
        ...
        self.menu_structure = {'Options':
                                   {'Edit URI': self.ep_window.show,
                                    "Show Log": lambda: print('show_log_window_opened'),
                                    'Exit': lambda: QtWidgets.qApp.exit(0)
                                    },
                               }

        self.button_names = {'add': self.add_row,
                             'edit request': lambda x: print('edit_request_window_open'),
                             'delete': self.delete_selected_row,
                             'send': lambda: print('send_fuc'),
                             }

        self.checkboxes_names = ["show JWT/UUIDs"]
        ...
        self.widget_config_1 = {'tables':  {'Requests':            (0, 0, 1, 3)},
                                'buttons': {'button_send':         (1, 0, 1, 3),
                                            'button_add':          (2, 0, 1, 1),
                                            'button_edit request': (2, 1, 1, 1),
                                            'button_delete':       (2, 2, 1, 1),
                                            },
                                'grids':   {'grid_Layout_2':       (4, 0, 1, 1, {'alignment': Qt.Qt.AlignLeft})}
                                }

        self.widget_config_2 = {'checkboxes': {'checkBox_showJWT/UUIDs': (4, 1, 1, 1)}
                                }
        self.configure()
    ...
    def table_field_configure(self, table_widget, names):
        table_widget = super().table_field_configure(table_widget,names)
    ...
    def configure(self):
        ...
        menu, submenus = self.create_window_menu(self.menu_structure, parent=window)
        central_widget = self.initialize_central_widget(window)
        self.central_widget_configure(window,menu,central_widget)

    def central_widget_configure(self, window, menu, central_widget):
        self.window.setMenuBar(menu)
    ...
    #
    #     self.model.add(self.model.item(* [combo_box.currentIndex()] + blank_data))